...
import { Link, CloudUpload, Loader2, AlertCircle, Check } from 'lucide-react';
      ...
      const interval = setInterval(async () => {
        try {
          ...
          if (statusData.status === 'progress' && statusData.percent_complete !== undefined) {
            ...
            // Keep UI in loading state but update progress
            setIsLoading(true);
  ...
  const handleProcess = async (ytLink: string | null = null, uploadedFile: File | null = null) => {
    ...
    try {
      ...
      if (result.task_id) {
        ...
        // Set state to indicate processing has started but is not complete
      ...
      // Only set loading to false if no active task is running
      if (!taskId) {
        setIsLoading(false);
      }
