...
@celery_app.task(bind=True)
def wan_2_1(self, prompt: str, email: str, aspect_ratio: str, image_url: str = None):
    """Generate video using wan 2.1 model via fal.ai"""
    try:
        # Update task status
        self.update_state(
            state="PROGRESS",
            meta={"status": "Generating video with wan 2.1...", "progress": 10},
        )

        # Prepare request parameters for wan 2.1
        request_data = {
            "prompt": prompt,
            "aspect_ratio": aspect_ratio,
        }
        ...
        if not result or "video" not in result:
            raise Exception("No video generated by wan 2.1 ")
        ...
        # Update user aura on failure (refund)
        users_collection.update_one(
            {"email": email}, {"$inc": {"aura": AURA_COSTS["wan 2.1"]}}
        )
        ...
        raise Exception(f"wan 2.1 generation failed: {str(e)}")
