...
from instagram_utils import download_instagram_post_image
...
def add_watermark(image_path: str, output_path: str) -> str:
    ...
    try:
        ...
        if watermarked.mode == "RGBA":
            ...
            rgb_image.paste(
                watermarked, mask=watermarked.split()[-1]
...
async def poll_job_status(
    client: httpx.AsyncClient, job_id: str, max_attempts: int = 60, delay: int = 5
):
    """Poll job status until completion or timeout"""
    headers = {"accept": "application/json", "Authorization": REMAKER_API_KEY}

    for attempt in range(max_attempts):
        try:
            response = await client.get(
                f"{REMAKER_FETCH_JOB_URL}/{job_id}", headers=headers
            )

            if response.status_code != 200:
                print(
                    f"Polling attempt {attempt + 1}: Status {response.status_code}, Response: {response.text}"
                )
            ...
            if code == 100000:
                ...
                if output_image_url and len(output_image_url) > 0:
                    ...
                    print(
                        f"Job {job_id} still processing (no output URL yet), waiting {delay} seconds..."
                    )
                    await asyncio.sleep(delay)
                    continue
            ...
            # Check if job is still in progress (these are the "in progress" codes)
                ...
                300101,
                300100,
            ]:  # Add other "in progress" codes as needed
                print(
                    f"Job {job_id} still processing (code: {code}), waiting {delay} seconds..."
                )
                await asyncio.sleep(delay)
                continue
    ...
    try:
        ...
        if youtubeUrl:
            try:
                ...
                return JSONResponse(
                    status_code=500,
                    content={"error": f"YouTube thumbnail error: {str(e)}"},
                )
            ...
            return JSONResponse(
                status_code=400,
                content={"error": "Provide either YouTube URL or thumbnail image."},
            )
        ...
        async with httpx.AsyncClient(timeout=300.0) as client:
            ...
            try:
                with open(thumbnail_path, "rb") as thumb_file, open(
                    face_path, "rb"
                ) as face_file:
                    files = {
                        "target_image": (
                    ...
                    response = await client.post(
                        REMAKER_CREATE_JOB_URL, headers=headers, files=files
                    )
            ...
            if not job_result:
                return JSONResponse(
                    status_code=500, content={"error": "Failed to get job result"}
                )
            ...
            try:
                ...
                if result_response.status_code != 200:
                    return JSONResponse(
                        status_code=500,
                        content={
                            "error": f"Failed to download result image: {result_response.status_code}"
                        },
                    )
            ...
            try:
                print(f"Starting S3 upload process. User paid: {user_paid}")

                if user_paid:
                    # User is paid - no watermark, upload original
                    s3_key = f"faceswap_results/{result_filename}"
                    print(f"Uploading original file to S3: {s3_key}")
                    ...
                    print(f"Successfully uploaded original to: {s3_urls['original']}")
                    ...
                    print("User is not paid, creating watermarked version...")
                    ...
                    watermarked_filename = (
                        f"{uuid.uuid4().hex}_watermarked_faceswap_output.jpg"
                    )
                    watermarked_path = os.path.join(TEMP_DIR, watermarked_filename)
                    print(f"Creating watermark: {watermarked_path}")
                    ...
                    if not os.path.exists(watermarked_path):
                        raise Exception(
                            f"Watermarked file not created: {watermarked_path}"
                        )

                    # 3. Upload watermarked version
                    watermarked_s3_key = (
                        f"faceswap_results/watermarked_{watermarked_filename}"
                    )
                    print(f"Uploading watermarked file to S3: {watermarked_s3_key}")
                    ...
                    print(
                        f"Successfully uploaded watermarked to: {s3_urls['watermarked']}"
                    )
                    ...
                    users_collection.update_one(
                        {"email": email},
                        {
                            "$push": {
                                "thumbnail": {
                                    "model": "faceswap",
                                    "watermarked_url": s3_urls[
                                        "watermarked"
                                    ],  # Return watermarked version to free user
                                    "original_url": s3_urls[
                                        "original"
                ...
                print(f"S3 upload process completed successfully. Main URL: {main_url}")
        ...
        return JSONResponse(
            status_code=500, content={"error": f"Face swap process failed: {str(e)}"}
        )
